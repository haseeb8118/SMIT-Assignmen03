...
//Display data is result variable contains value
function displayData(results){
  debugger;
if(results.length === 0){
  para.display = "block";para.innerHTML = `We don't have enough resources. Please select specific data from dropdown`;
  table.innerHTML =""
}
else{
  table.innerHTML =" ";
    ...
    para.innerHTML = ``
     table.innerHTML +=  `
       ...
       <td class="px-6 py-3 flex"> <div><img class="px-2 inline-block" src="${'https://image.tmdb.org/t/p/w45' + movie.poster_path}" />
            ...
            <div>
            <div class="font-semibold text-xl text-blue-500 pb-2">
            ${movie.title}</div>
            <button class="bg-gray-100 border-black-500 border-2 text-blackfont-bold text-center p-[1px] mr-2 rounded">${movie.certification}</button> ${movie.genres.toString(" , ")} • ${hr}h ${min}m</div>
            </td>
